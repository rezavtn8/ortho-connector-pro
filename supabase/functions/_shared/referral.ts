/**
 * Deno-side copy of `src/lib/officeMetrics.ts`.
 *
 * WHY THIS IS DUPLICATED, AND WHAT KEEPS IT HONEST
 *
 * The assistant has to answer "who is slipping" with the same numbers the Offices
 * table, the map and the briefing rail show. If the edge function invented its own
 * definition of VIP or of `slipping`, the assistant would confidently contradict the
 * screen next to it — the single fastest way to make a data product untrustworthy.
 *
 * Supabase bundles an edge function from its own directory, so the function cannot
 * import out of `src/`. The body below is therefore a verbatim copy, and
 * `src/lib/__tests__/referralParity.test.ts` imports both this file and the original
 * and asserts they agree across a fixture set. Change one, the parity test fails, and
 * you are told to change the other. Do not edit this file alone.
 */
