...
export function PatientFlowMap({
  ...
  // Land on the deep-linked month if it exists, otherwise the most recent.
  useEffect(() => {
    if (months.length === 0 || monthIndex !== null) return;
    const wanted = initialMonth ? months.indexOf(initialMonth) : -1;
    setMonthIndex(wanted >= 0 ? wanted : months.length - 1);
  }, [months, monthIndex, initialMonth]);
